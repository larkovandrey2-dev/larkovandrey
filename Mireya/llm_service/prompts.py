...
GAMING_PROMPTS = {9:[
    "Проанализируй ответ про компьютерные игры. Определи, как он играет в игры: если в одиночку - поставь 0, если любым другим способом поставь число 1. Если непонятно, верни -1.\nВопрос: {question}\nОтвет: {answer}",
    "Классифицируй гейминг пользователя: 0=играет в одиночку, 1-3=играет с друзьями, либо каким-то другим способом. Если непонятно, верни -1.Верни только цифру соответствия.\nВопрос: {question}\nОтвет: {answer}"],
    10:["Определи по ответу пользователя, на каком устройстве он играет в игры: 0-ПК, компьютер, 1-консоль(playstation,xbox,nintendo и другие), 2-мобильный телефон или портативное устройство. Если непонятно, верни -1.Верни только цифру соответствия.\nВопрос: {question}\nОтвет: {answer}"],
    11:["Определи по ответу, любимую или предпочтительную игру для пользователя, верни только название игры или Other, если ты не смог определить игру. Если непонятно ничего, верни -1. Верни только цифру соответствия.\nВопрос: {question}\nОтвет: {answer}"]
}
...
REPHRASE_PROMPTS = [
    "Переформулируй следующий вопрос, сохранив его точный смысл. Сделай его немного более живым и разговорным и обращайся на ты, как с другом, но не используй слова типа 'Братан и друг', будь уважительнее, а также убери из вопроса просьбу об оценки цифрами, если она есть. Не используй сложные обороты. Вопрос: {question}",
    "Измени формулировку этого вопроса, чтобы он звучал мягче и участливее, но смысл остался тем же и обращайся на ты, как с другом, но не используй слова типа 'Братан и друг', будь уважительнее, а также убери из вопроса просьбу об оценки цифрами, если она есть. Вопрос: {question}",
    "Задай этот же вопрос другими словами. Используй дружелюбный тон и обращайся на ты, как с другом, но не используй слова типа 'Братан и друг', будь уважительнее, а также убери из вопроса просьбу об оценки цифрами, если она есть. Вопрос: {question}",
    "Представь этот вопрос немного иначе, чтобы разнообразить диалог. Сохрани суть и обращайся на ты, как с другом, но не используй слова типа 'Братан и друг', будь уважительнее, а также убери из вопроса просьбу об оценки цифрами, если она есть. Вопрос: {question}"
]
...
def get_analysis_prompt(question: str, answer: str, question_n: int, survey_n: int = 1) -> str:
    ...
    if survey_n == 1:
        if question_n <= 7:
            ...
            template = GAMING_PROMPTS[question_n][question_n % len(GAMING_PROMPTS[question_n])]
    elif survey_n == 2:
